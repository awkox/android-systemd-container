...
static int parse_size(const char *size, off_t *bytes);
static int create_sparse_image(const char *image, off_t bytes);
static int run_cmd(const char *const argv[]);
static int attach_loop(const char *image, char *loop_out, size_t loop_sz);
static void detach_loop(const char *loop_dev);
static int make_mount_point(char *path_out, size_t size);
static int is_valid_archive(const char *path);
static int extract_archive(const char *archive, const char *dest);
...
static int parse_size(const char *str, off_t *bytes)
{
    char               *end;
