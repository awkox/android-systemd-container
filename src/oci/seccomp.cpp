...
public:
  ...
  void stmt(uint16_t code, uint32_t k) { filter.push_back(BPF_STMT(code, k)); }
...
int seccomp_apply_minimal(const int privileged_mask) {
   ...
   * 6. 阻止宿主机时钟修改
   */
  bpf.deny_syscall(SYS_settimeofday,
                   SECCOMP_RET_ERRNO | (EPERM & SECCOMP_RET_DATA));
  bpf.deny_syscall(SYS_adjtimex,
                   SECCOMP_RET_ERRNO | (EPERM & SECCOMP_RET_DATA));
  bpf.deny_syscall(SYS_clock_settime,
                   SECCOMP_RET_ERRNO | (EPERM & SECCOMP_RET_DATA));
  bpf.deny_syscall(SYS_clock_adjtime,
                   SECCOMP_RET_ERRNO | (EPERM & SECCOMP_RET_DATA));
...
int android_seccomp_setup(const bool block_nested_ns,
  ...
  if (privileged_mask & PRIV_NOSEC) {
    if (block_nested_ns) {
      log_warn(
          "由于启用了 privileged=noseccomp，block-nested-namespaces 已失效。");
    }
    return 0;
  }
  ...
  if (major < 5) {
    bpf.deny_syscall(SYS_keyctl,
                     SECCOMP_RET_ERRNO | (ENOSYS & SECCOMP_RET_DATA));
  }

  if (block_nested_ns) {
    log_info("[SEC] 激活 block-nested-namespaces: "
             "已强制拦截后续的命名空间系统调用。");
  ...
  if (bpf.apply() < 0) {
    log_error("由于未知错误无法应用 Android 附加 Seccomp 滤网: {}",
              strerror(errno));
    return -1;
  }
