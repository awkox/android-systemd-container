...
constexpr auto universal_drops = std::to_array<int>({CAP_SYS_MODULE});
...
void apply_capability_hardening(const int privileged_mask) {
  ...
  auto drop_caps = [&](const auto &caps) -> int {
    ...
    for (const int cap : caps) {
      ...
      // 2. 【修复 #2】同步从当前进程的 Effective, Permitted 和 Inheritable
      // 集合中抹除 因为如果不这么做，当前进程仍持有这些能力，安全性等同于没做
      if (cap >= 0 && cap < 64) {
  ...
  if (syscall(SYS_capset, &hdr, data) < 0) {
    log_warn("[SEC] 无法应用 Capabilities 裁剪策略 (capset): {}",
             strerror(errno));
  }

  log_info("[SEC] 已完成内核权限边界裁剪 (共移除了 {} 个 Cap)。",
           total_dropped);
}
