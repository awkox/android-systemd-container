...
bool setup_procfs(const asc::conf &cfg) {
  if (mount("proc", "proc", "proc", MS_NOSUID | MS_NODEV | MS_NOEXEC, nullptr) <
      0) {
...
bool setup_virtual_filesystems(asc::rt &rt) {
  ...
  if (mount("none", "dev", "tmpfs", MS_NOSUID | MS_NOEXEC, "size=8M,mode=755") <
      0)
    return false;
  if (setup_dev() < 0) {
  ...
  if (mount(rt.console.name.c_str(), "dev/console", nullptr, MS_BIND, nullptr) <
      0)
    log_warn("无法绑定挂载 Console '{}': {}", rt.console.name.c_str(),
             strerror(errno));
...
void execute_init_process(asc::rt &rt) {
  const char *init_bin =
      rt.cfg.custom_init.empty() ? DEFAULT_INIT : rt.cfg.custom_init.c_str();
  const char *init_args[] = {init_bin, "systemd.unified_cgroup_hierarchy=1",
                             nullptr};
  ...
  if (execve(init_bin, const_cast<char *const *>(init_args),
             const_cast<char *const *>(environment)) < 0) {
...
void internal_boot(asc::rt &rt) {
  ...
  // 1. 挂载传播隔离与 Rootfs 挂载
  if (!setup_mount_isolation_and_rootfs(rt))
    return;

  // 2. 构建 proc, sys, dev 虚拟文件系统
  if (!setup_virtual_filesystems(rt))
    return;

  // 3. 切换根目录
  if (!execute_pivot_root())
    return;

  // 4. 调整根目录标志与主机名
  if (!setup_system_environment())
    return;
  ...
  asc::oci::android_seccomp_setup(rt.cfg.block_nested_ns,
                                  rt.cfg.privileged_mask);
